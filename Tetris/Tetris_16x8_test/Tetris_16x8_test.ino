...
const byte SW_Pin[] = {A0, A3, A2, A1, A4}, RGB_Pin[] = {10, 9, 6}, tone_Pin[] = {5, 7};  //按鍵腳位, LED腳位, 蜂鳴器腳位
...
void setup() {
  ...
  lc.setIntensity(0, 8); //螢幕亮度調整
  lc.setIntensity(1, 8); //螢幕亮度調整
  lc.setIntensity(2, 15); //螢幕亮度調整
  lc.setRow(2, 0, num[0]);
  lc.setRow(2, 1, num[0]);
  lc.setRow(2, 2, num[0]);
  for (byte a = 0; a < 5; a++) {
...
void loop() {
  ...
  if (down()) { //執行下降的動作並判斷如果停止下降就執行下列動作
    ...
    if (Gameover) { //如果遊戲結束執行下列動作
      ...
      lcd_dis();
      //----------
      byte Score_var = 0; //此變數為清除的行數
      //計算清除橫行並將清除的空間填入
      for (byte b = 3; b < 19; b++) {
        if (clr[b - 3]) {
          for (byte c = 0; c < b - 2; c++)
            for (byte a = 0; a < 8; a++)
              lcd[b - c][a] = lcd[b - 1 - c][a];
          Score_var++;
        }
      }
      //--------------------------------
...
void SW() {
  if (digitalRead(SW_Pin[4])) {
    ...
    if (!digitalRead(SW_Pin[1]) || !digitalRead(SW_Pin[2])) {
      if (digitalRead(SW_Pin[1]) && Score != 999) {
        score_press = 1;
...
//七段顯示分數並控制遊戲難度
void score_dis(int score) {
  if (score > 100) speed_time = 100;
  else speed_time = map(score, 0, 100, 1000, 100);
  if (score > 999) score = 999;
  lc.setRow(2, 0, num[score % 10]);
  lc.setRow(2, 1, num[score / 10 % 10]);
  lc.setRow(2, 2, num[score / 100 % 10]);
}
